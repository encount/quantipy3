...
class TestRules(unittest.TestCase):
    ...
    def test_sortx_summaries_items(self):
        ...
        for vk in vks:
            ...
            if not 'd.mean' in vk and not 'cbase' in vk:
                check_chain_view_dataframe = v.dataframe.reindex_like(
                    l.dataframe)
                self.assertTrue(check_chain_view_dataframe.equals(l.dataframe))
                actual_order = v.dataframe.index.get_level_values(1).tolist()
    ...
    def test_sortx(self):
        ...
        rules_values_x = {
            'unwtd': index_items(col_x, all=True,
                                 values=[2, 1, 3, 15, 4, 5, 16, 6, 10, 12, 14,
                                         11, 7, 13, 8, 9]),
            'iswtd': index_items(col_x, all=True,
                                 values=[2, 1, 3, 15, 4, 5, 16, 6, 12, 10, 14,
                                         11, 7, 13, 9, 8])
        }

        rules_values_y = {
            'unwtd': index_items(col_y, all=True,
                                 values=[1, 2, 16, 7, 15, 12, 3, 11, 14, 6, 8,
                                         10, 9, 5, 4, 13]),
            'iswtd': index_items(col_y, all=True,
                                 values=[1, 2, 16, 7, 12, 11, 3, 15, 8, 9, 10,
                                         14, 5, 6, 4, 13])
        }
        ...
        rules_values_x = {
            'unwtd': index_items(col_x, all=True,
                                 values=[2, 15, 4, 16, 6, 10, 12, 14, 11, 7, 13,
                                         8, 9, 5, 1, 3]),
            'iswtd': index_items(col_x, all=True,
                                 values=[2, 15, 4, 16, 6, 12, 10, 14, 11, 7, 13,
                                         9, 8, 5, 1, 3])
        }

        rules_values_y = {
            'unwtd': index_items(col_y, all=True,
                                 values=[1, 16, 7, 15, 12, 3, 11, 14, 8, 10, 9,
                                         5, 13, 6, 2, 4]),
            'iswtd': index_items(col_y, all=True,
                                 values=[1, 16, 7, 12, 11, 3, 15, 8, 9, 10, 14,
                                         5, 13, 6, 2, 4])
        }
        ...
        rules_values_x = {
            'unwtd': index_items(col_x, all=True,
                                 values=[2, 1, 3, 15, 4, 5, 16, 12, 6, 10, 14,
                                         11, 7, 13, 9, 8]),
            'iswtd': index_items(col_x, all=True,
                                 values=[2, 1, 3, 15, 4, 5, 16, 12, 6, 10, 14,
                                         11, 7, 13, 9, 8])
        ...
        rules_values_y = {
            'unwtd': index_items(col_y, all=True,
                                 values=[1, 2, 16, 7, 11, 3, 12, 15, 8, 9, 10,
                                         5, 14, 6, 4, 13]),
            'iswtd': index_items(col_y, all=True,
                                 values=[1, 2, 16, 7, 11, 3, 12, 15, 8, 9, 10,
                                         5, 14, 6, 4, 13])
    ...
    def test_rules_frequency(self):
        ...
        rules_values_y = {
            'unwtd': index_items(col, all=True,
                                 values=[1, 3, 15, 5, 16, 10, 12, 14, 11, 7, 13,
                                         8, 9, 6, 2, 4]),
            'iswtd': index_items(col, all=True,
                                 values=[1, 3, 15, 5, 16, 12, 10, 14, 11, 7, 13,
                                         9, 8, 6, 2, 4])
        }
        ...
        rules_values_x = {
            'unwtd': index_items(col, all=True,
                                 values=[4, 5, 6, 10, 12, 11, 7, 13, 8, 9, 1,
                                         2]),
            'iswtd': index_items(col, all=True,
                                 values=[4, 5, 6, 12, 10, 11, 7, 13, 9, 8, 1,
                                         2])
        }
        ...
        rules_values_x = {
            'unwtd': index_items(col, all=True,
                                 values=[3, 15, 4, 16, 6, 10, 12, 14, 7, 8, 9,
                                         1, 2]),
            'iswtd': index_items(col, all=True,
                                 values=[3, 15, 4, 16, 6, 12, 10, 14, 7, 9, 8,
                                         1, 2])
        }

        rules_values_y = {
            'unwtd': index_items(col, all=True,
                                 values=[2, 1, 3, 4, 5, 6, 10, 12, 11, 8, 9, 15,
                                         16]),
            'iswtd': index_items(col, all=True,
                                 values=[2, 1, 3, 4, 5, 6, 12, 10, 11, 9, 8, 15,
                                         16])
        }
    ...
    def test_rules_crosstab(self):
        ...
        if EXTENDED_TESTS:
            ...
            rules_values_x = {
                'unwtd': index_items(col_x, all=True,
                                     values=[2, 15, 4, 16, 6, 10, 12, 14, 11, 7,
                                             13, 8, 9, 5, 1, 3]),
                'iswtd': index_items(col_x, all=True,
                                     values=[2, 15, 4, 16, 6, 12, 10, 14, 11, 7,
                                             13, 9, 8, 5, 1, 3])
            }

            rules_values_y = {
                'unwtd': index_items(col_y, all=True,
                                     values=[1, 16, 7, 15, 12, 3, 11, 14, 8, 10,
                                             9, 5, 13, 6, 2, 4]),
                'iswtd': index_items(col_y, all=True,
                                     values=[1, 16, 7, 12, 11, 3, 15, 8, 9, 10,
                                             14, 5, 13, 6, 2, 4])
            }
            ...
            rules_values_x = {
                'unwtd': index_items(col_x, all=True,
                                     values=[5, 6, 10, 12, 11, 13, 8, 9, 4, 7,
                                             3]),
                'iswtd': index_items(col_x, all=True,
                                     values=[5, 6, 12, 10, 11, 13, 9, 8, 4, 7,
                                             3])
            }

            rules_values_y = {
                'unwtd': index_items(col_y, all=True,
                                     values=[16, 15, 12, 14, 8, 10, 9, 7, 11,
                                             13]),
                'iswtd': index_items(col_y, all=True,
                                     values=[16, 12, 15, 8, 9, 10, 14, 7, 11,
                                             13])
            }
            ...
            rules_values_x = {
                'unwtd': index_items(col_x, all=True,
                                     values=[2, 1, 15, 16, 6, 12, 14, 11, 13, 8,
                                             9, 4, 7, 3]),
                'iswtd': index_items(col_x, all=True,
                                     values=[2, 1, 15, 16, 6, 12, 14, 11, 13, 9,
                                             8, 4, 7, 3])
            }

            rules_values_y = {
                'unwtd': index_items(col_y, all=True,
                                     values=[1, 2, 16, 15, 3, 14, 6, 8, 10, 9,
                                             5, 7, 11, 13]),
                'iswtd': index_items(col_y, all=True,
                                     values=[1, 2, 16, 3, 15, 8, 9, 10, 14, 5,
                                             6, 7, 11, 13])
            }
...
def confirm_crosstabs(self, meta, data,
    ...
    for weight in weights:
        ...
        for xtotal in [False, True]:
            ...
            # rules=xy
            df = crosstab(meta, data, col_x, col_y, weight=weight,
                          rules=['x', 'y'], xtotal=xtotal)
            confirm_index_columns(self, df, rules_x, rules_y)
...
def confirm_get_chain(self,
    ...
    weight = None
    chain_true_unwtd = stack.get_chain(x=xks, y=yks, views=vks, rules=True,
                                       rules_weight=weight)
    chain_false_unwtd = stack.get_chain(x=xks, y=yks, views=vks, rules=False,
                                        rules_weight=weight)
    chain_x_unwtd = stack.get_chain(x=xks, y=yks, views=vks, rules=['x'],
                                    rules_weight=weight)
    chain_y_unwtd = stack.get_chain(x=xks, y=yks, views=vks, rules=['y'],
                                    rules_weight=weight)
    chain_xy_unwtd = stack.get_chain(x=xks, y=yks, views=vks, rules=['x', 'y'],
                                     rules_weight=weight)

    weight = 'weight_a'
    chain_true_wtd = stack.get_chain(x=xks, y=yks, views=vks, rules=True,
                                     rules_weight=weight)
    chain_false_wtd = stack.get_chain(x=xks, y=yks, views=vks, rules=False,
                                      rules_weight=weight)
    chain_x_wtd = stack.get_chain(x=xks, y=yks, views=vks, rules=['x'],
                                  rules_weight=weight)
    chain_y_wtd = stack.get_chain(x=xks, y=yks, views=vks, rules=['y'],
                                  rules_weight=weight)
    chain_xy_wtd = stack.get_chain(x=xks, y=yks, views=vks, rules=['x', 'y'],
                                   rules_weight=weight)
            ...
            for vk in vks:
                ...
                for weight in [None, 'weight_a']:
                    ...
                    if weight is None:
                        ...
                        df = get_dataframe(chain_true_unwtd, keys=keys,
                                           rules=False)
                        #                         print df
                        #                         print df.index
                        #                         print df.columns
                        ...
                        # rules=False
                        df = get_dataframe(chain_false_unwtd, keys=keys,
                                           rules=False)
                        confirm_index_columns(self, df, natural_x, natural_y)

                        # rules=x
                        df = get_dataframe(chain_x_unwtd, keys=keys,
                                           rules=False)
                        confirm_index_columns(self, df, rules_x, natural_y)

                        # rules=y
                        df = get_dataframe(chain_y_unwtd, keys=keys,
                                           rules=False)
                        confirm_index_columns(self, df, natural_x, rules_y)

                        # rules=xy
                        df = get_dataframe(chain_xy_unwtd, keys=keys,
                                           rules=False)
                        confirm_index_columns(self, df, rules_x, rules_y)

                    else:
                        df = get_dataframe(chain_true_wtd, keys=keys,
                                           rules=False)
                        #                         print df
                        #                         print df.index
                        #                         print df.columns
                        ...
                        # rules=False
                        df = get_dataframe(chain_false_wtd, keys=keys,
                                           rules=False)
                        confirm_index_columns(self, df, natural_x, natural_y)
...
def get_xy_values(meta, data,
    ...
    condensed_x = relation.split(":")[0].startswith('x') or v_method.startswith(
        'd.')
