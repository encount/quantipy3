...
def paint_box(worksheet, frames, format_dict, rows, cols, metas, formats_spec,
              has_weighted_views=False, y_italicise=dict(), ceil=False,
              floor=False,
        ...
        if method == 'dataframe_columns':
            ...
            # coltests
                ...
                if relation == ':' or ('t.props' not in fullname.split('|')[1]):
                    ...
                    net_bg_color_user = format_dict[test_key].__dict__[
                        'bg_color']
            ...
            if not is_array:
                if (cols[0] == cols[1]):
                    ...
                    worksheet.merge_range(
                            row, cols[0], row, cols[1], labels[0][0],
                            existing_format
                    )
    ...
    for xy in x_iter[chain.orientation]:
        ...
        if dummy_tests:
            ...
            for vk in offset_dict[xy]:
                if not vk.split('|')[-1].startswith(
                        'cbase') and vk not in exempt:
                    v_type = vk.split('|')[1][0]
...
def get_cell_details(views, default_text=None, testcol_maps={},
                     group_order=None):
    global CD_TRANSMAP
        ...
        if all(conditions):
            ...
            for chain in chain_generator(cluster):
                ...
                for column in chain_format.columns.tolist():
                    ...
                    if not has_multiindex:
                        if meta['columns'][column]['type'] in ['single']:
                        ...
                        elif meta['columns'][column]['type'] in [
                            'delimited set']:
                            categories = {
                            ...
                            #                         series.fillna('')
                            series[series.columns[0]] = series[
                                series.columns[0]].str.cat(
                ...
                if has_multiindex:
                    ...
                    for level, _ in it:
                        worksheet.write(7 + lrow, 0, level,
                                        formats['x_left_bold'])
                        for idx in df.loc[level].index:
                            worksheet.write(7 + lrow, 1, idx,
                                            formats['x_right'])
                            lrow += 1
                            ...
                            if column != '@':
                                ...
                                if column not in testcol_maps:
                                    ...
                                    if helpers.is_mapped_meta(values):
                                        values = helpers.emulate_meta(meta,
                                                                      values)
                                    y_values = [
                                        int(v)
                                        for v in
                                        list(zip(*[c for c in df.columns]))[1]]
                                    values = [
                                        [
            ...
            for chain in chain_generator(cluster):
                ...
                for xy in xy_generator(chain):
                    ...
                    # fill xs' ceil_floor
                    ceiling, _ = min(iter(offset[x].items()),
                                     key=lambda o: o[1])
                    ...
                    if extract_mask_label and chain.content_of_axis.index(
                            xy) == 0:
                        #                         mask_label_rows = 0
                    ...
                    for vi, views in enumerate(
                        ...
                        for idx, v in enumerate(views):
                            ...
                            if view.meta()['agg']['method'] == 'frequency':
                                ...
                                if all(conditions_1):
                                    ...
                                    if any(view.meta()[axis]['is_array'] for
                                           axis in ['x', 'y']):
                                        df = helpers.paint_view(
                                                meta=meta,
                                                view=view,
                        ...
                        if dummy_tests:
                            conditions = [
                                ...
                                len(frames) > 1 and not any(
                                        ...
                                        for vm in vmetas)]
                            if any(conditions):
                                if not vmetas[0]['agg']['name'].startswith(
                                        'cbase'):
                        ...
                        if y_name == '@' and not is_array:
                            ...
                            if x_name in meta['columns']:
                                if meta['columns'][x_name].get('parent'):
                                    parent = list(meta['columns'][x_name][
                                                      'parent'].keys())[
                                        0].split('@')[1]
                        ...
                        if df_cols[0][0] == col_index_origin:
                            if fullname == ceiling:
                                ...
                                if extract_mask_label:
                                    if meta['columns'].get(xk, {}).get(
                                            'parent'):
                                        as_mask = list(meta['columns'][xk][
                                                           'parent'].keys())[
                                            0].split('@')[-1]
                                    else:
                                        as_mask = re.sub('\[.+?\]', '', xk)
                        ...
                        if is_array:
                            if vi == 0:
                                format_key = 'x_right'
                                labels = [df.index.levels[1][i] for i in
                                          df.index.labels[1]]
                            ...
                            if shortname.startswith('cbase'):
                                if has_weighted_views and not is_weighted:
                                    ...
                                    if len(text) > 0:
                                        if chain.base_text is not None and \
                                                vmetas[idx]['agg'][
                                                    'add_base_text']:
                                ...
                                if (vmetas[0]['agg']['method'] in [
                                    'descriptives'] or
                                        (vmetas[0]['agg']['method'] in [
                                            'frequency'] and not relation in [
                                            ':', 'x++:'])):
                                    if len(frames) > 1:
                                        labels = []
                                        labels_written = []
                                        for idxdf, df in enumerate(frames):
                                            if vmetas[idxdf]['agg'][
                                                ...
                                                if not vmetas[idxdf]['agg'][
                                                    'is_block']:
                                                    if len(vmetas[idxdf]['agg'][
                                                               'text']) > 0:
                                                        if isinstance(vmetas[0][
                                                                          'agg'][
                                                                          'text'],
                                                                      str):
                                                            if vmetas[0]['agg'][
            ...
            # set y axis height
            worksheet.set_row(row_index_origin - 3,
                              formats_spec.y_header_height)
    ...
    for i in range(len(toc_names)):
        for q in range(len(toc_names[i])):
            TOCsheet.write(
                    ...
                    'internal:%s!%s' % (
                        names[i],
                        xl_rowcol_to_cell(toc_locs[i][q][0],
                                          toc_locs[i][q][1])),
                    formats['TOC-url'])
            TOCsheet.write(
                    6 + q,
