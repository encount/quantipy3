...
def rename_duplicate_shape_names(pres_path, overwrite=True):
    ...
    for slide in prs.slides:
        ...
        renamed_shapes = [x + "_" + str(i) if shape_names.count(x) > 1 else x
                          for i, x in enumerate(shape_names)]
    ...
    for i, sld in enumerate(prs.slides, start=1):
        if slide_num:
            if i in slide_num:
                ...
                pptx_tree[file_name]['slides'][slide_number][
                    'slide layout'] = OrderedDict()
                slide_layout_name = str(sld.slide_layout.name)
                pptx_tree[file_name]['slides'][slide_number]['slide layout'][
                ...
                for x, shp in enumerate(sld.shapes):
                    ...
                    shape_number = str(x)
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number] = OrderedDict()
                    ...
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number]['shape name'] = shape_name
                    ...
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number]['shape type'] = shape_type
                    ...
                    if str(shp.shape_type) == 'PLACEHOLDER (14)':
                        ...
                        pptx_tree[file_name]['slides'][slide_number]['shapes'][
                            shape_number]['placeholder idx'] = str(
                            shp.placeholder_format.idx)
                        pptx_tree[file_name]['slides'][slide_number]['shapes'][
                            shape_number]['placeholder type'] = str(
                            shp.placeholder_format.type)
                    ...
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number]['shape dimensions'] = OrderedDict()
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number]['shape dimensions']['left'] = str(
                        shp.left)
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number]['shape dimensions']['top'] = str(shp.top)
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number]['shape dimensions']['width'] = str(
                        shp.width)
                    pptx_tree[file_name]['slides'][slide_number]['shapes'][
                        shape_number]['shape dimensions']['height'] = str(
                        shp.height)
            ...
            for x, shp in enumerate(sld.shapes):
                ...
                shape_id = str(shp.id)
                pptx_tree[file_name]['slides'][slide_number]['shapes'][
                    shape_number]['shape id'] = shape_id
                ...
                pptx_tree[file_name]['slides'][slide_number]['shapes'][
                    shape_number]['shape dimensions'] = OrderedDict()
                pptx_tree[file_name]['slides'][slide_number]['shapes'][
                    shape_number]['shape dimensions']['left'] = str(shp.left)
                pptx_tree[file_name]['slides'][slide_number]['shapes'][
                    shape_number]['shape dimensions']['top'] = str(shp.top)
                pptx_tree[file_name]['slides'][slide_number]['shapes'][
                    shape_number]['shape dimensions']['width'] = str(shp.width)
                pptx_tree[file_name]['slides'][slide_number]['shapes'][
                    shape_number]['shape dimensions']['height'] = str(
                    shp.height)
...
def read_slide(sld):
    ...
    for x, shp in enumerate(sld.shapes):
        ...
        print('{indent:>10}shape name - {shape_name}'.format(indent='',
                                                             shape_name=shp.name))
        print('{indent:>15}shape type - {shape_type}'.format(indent='',
                                                             shape_type=shp.shape_type))
        if str(shp.shape_type) == 'PLACEHOLDER (14)':
            print(
                '{indent:>15}placeholder idx - {placeholder_idx}, placeholder type - {placeholder_type}'.
                format(indent='', placeholder_idx=shp.placeholder_format.idx,
                       placeholder_type=shp.placeholder_format.type))
        print(
            '{indent:>15}shape dimensions - left ({shape_left}), top ({shape_top}), height ({shape_height}), width ({shape_width})\n'.
            format(indent='', shape_left=shp.left, shape_top=shp.top,
                   shape_height=shp.height, shape_width=shp.width))
        ...
        if i == slide_num:
            for x, shp in enumerate(sld.shapes):
                ...
                if shp.name == chart_name:
                    ...
                    print('chart >\n')
                    print('   chart_style: {chart_style}'.format(
                        chart_style=shp.chart.chart_style))
                    print('   has_legend: {has_legend}'.format(
                        has_legend=shp.chart.has_legend))
                    print(
                    ...
                    print(
                        '   has_major_gridlines: {has_major_gridlines}'.format(
                            has_major_gridlines=caxis.has_major_gridlines))
                    print(
                        '   has_minor_gridlines: {has_minor_gridlines}'.format(
                            has_minor_gridlines=caxis.has_minor_gridlines))
                    print('   major_tick_mark: {major_tick_mark}'.format(
                        major_tick_mark=caxis.major_tick_mark))
                    print('   maximum_scale: {maximum_scale}'.format(
                        maximum_scale=caxis.maximum_scale))
                    print('   minimum_scale: {minimum_scale}'.format(
                        minimum_scale=caxis.minimum_scale))
                    print('   minor_tick_mark: {minor_tick_mark}'.format(
                        minor_tick_mark=caxis.minor_tick_mark))
                    print('   tick_labels: {tick_labels}'.format(
                        tick_labels=str(caxis.tick_labels)))
                    print(
                        '   tick_label_position: {tick_label_position}'.format(
                            tick_label_position=caxis.tick_label_position))
                    print('   tick_labels_font_name: {tick_labels_font}'.format(
                        tick_labels_font=caxis.tick_labels.font.name))
                    print('   tick_labels_font_size: {tick_labels_font}'.format(
                        tick_labels_font=caxis.tick_labels.font.size))
                    print('   tick_labels_font_bold: {tick_labels_font}'.format(
                        tick_labels_font=caxis.tick_labels.font.bold))
                    print(
                        '   tick_labels_font_color: {tick_labels_font}'.format(
                            tick_labels_font=caxis.tick_labels.font.color))
                    print(
                        '   tick_labels_font_italic: {tick_labels_font}'.format(
                            tick_labels_font=caxis.tick_labels.font.italic))
                    print(
                        '   tick_labels_font_underline: {tick_labels_font}'.format(
                            tick_labels_font=caxis.tick_labels.font.underline))
                    print(
                        '   tick_labels_number_format: {tick_labels_number_format}'.format(
                            tick_labels_number_format=caxis.tick_labels.number_format))
                    print(
                        '   tick_labels_number_format_is_linked: {tick_labels_number_format_is_linked}'.format(
                            tick_labels_number_format_is_linked=caxis.tick_labels.number_format_is_linked))
                    print('   tick_labels_offset: {tick_labels_offset}'.format(
                        tick_labels_offset=caxis.tick_labels.offset))
                    print(
                    ...
                    print(
                        '   has_major_gridlines: {has_major_gridlines}'.format(
                            has_major_gridlines=vaxis.has_major_gridlines))
                    print(
                        '   has_minor_gridlines: {has_minor_gridlines}'.format(
                            has_minor_gridlines=vaxis.has_minor_gridlines))
                    print('   major_tick_mark: {major_tick_mark}'.format(
                        major_tick_mark=vaxis.major_tick_mark))
                    print('   maximum_scale: {maximum_scale}'.format(
                        maximum_scale=vaxis.maximum_scale))
                    print('   minimum_scale: {minimum_scale}'.format(
                        minimum_scale=vaxis.minimum_scale))
                    print('   major_unit: {major_unit}'.format(
                        major_unit=vaxis.major_unit))
                    print('   minor_unit: {minor_unit}'.format(
                        minor_unit=vaxis.minor_unit))
                    print('   minor_tick_mark: {minor_tick_mark}'.format(
                        minor_tick_mark=vaxis.minor_tick_mark))
                    print('   tick_labels: {tick_labels}'.format(
                        tick_labels=vaxis.tick_labels))
                    print(
                        '   tick_label_position: {tick_label_position}'.format(
                            tick_label_position=vaxis.tick_label_position))
                    print('   tick_labels_font_name: {tick_labels_font}'.format(
                        tick_labels_font=vaxis.tick_labels.font.name))
                    print('   tick_labels_font_size: {tick_labels_font}'.format(
                        tick_labels_font=vaxis.tick_labels.font.size))
                    print('   tick_labels_font_bold: {tick_labels_font}'.format(
                        tick_labels_font=vaxis.tick_labels.font.bold))
                    print(
                        '   tick_labels_font_color: {tick_labels_font}'.format(
                            tick_labels_font=vaxis.tick_labels.font.color))
                    print(
                        '   tick_labels_font_italic: {tick_labels_font}'.format(
                            tick_labels_font=vaxis.tick_labels.font.italic))
                    print(
                        '   tick_labels_font_underline: {tick_labels_font}'.format(
                            tick_labels_font=vaxis.tick_labels.font.underline))
                    print('   tick_labels_font: {tick_labels_font}'.format(
                        tick_labels_font=vaxis.tick_labels.font))
                    print(
                        '   tick_labels_number_format: {tick_labels_number_format}'.format(
                            tick_labels_number_format=vaxis.tick_labels.number_format))
                    print(
                        '   tick_labels_number_format_is_linked: {tick_labels_number_format_is_linked}'.format(
                            tick_labels_number_format_is_linked=vaxis.tick_labels.number_format_is_linked))
                    print(
                    ...
                    for item in shp.chart.plots:
                        ...
                        print('chart > plot\n')
                        print('   plot_categories: {plot_cats}'.format(
                            plot_cats=item.categories))
                        print('   plot_gap_width: {gap_width}'.format(
                            gap_width=item.gap_width))
                        print('   has_data_labels: {has_data_labels}'.format(
                            has_data_labels=item.has_data_labels))
                        print('   overlap: {overlap}'.format(
                            overlap=item.overlap))
                        print('   vary_by_categories: {vary_by_cat}\n'.format(
                            vary_by_cat=item.vary_by_categories))
                        print('-' * 110)
                        ...
                        print('   data_label_font_name: {font_name}'.format(
                            font_name=font.name))
                        print('   data_label_font_size: {font_size}'.format(
                            font_size=font.size))
                        print(
                            '   data_label_font_bold: {data_label_font}'.format(
                                data_label_font=font.bold))
                        print('   data_label_font_color {font_color}'.format(
                            font_color=font.color))
                        print('   data_label_font_fill {font_fill}'.format(
                            font_fill=font.fill))
                        print('   data_label_font_italic: {font_italic}'.format(
                            font_italic=font.italic))
                        print(
                            '   data_label_font_underline: {font_underline}\n'.format(
                                font_underline=font.underline))
                        print('-' * 110)

                        for ser in item.series:
                            print('chart > plot > series\n')
                            print('   series_fill_type: {fill_type}'.format(
                                fill_type=ser.fill.type))
                            print(
                                '   series_invert_if_neg: {invert_if_neg}'.format(
                                    invert_if_neg=ser.invert_if_negative))
                            print(
                                '   series_line: {line}'.format(line=ser.line))
                            print(
                                '   series_name: {name}'.format(name=ser.name))
                            print('   series_values: {values}'.format(
                                values=ser.values))
            ...
            for x, shp in enumerate(sld.shapes):
                ...
                if shp.name == chart_name:
                    ...
                    for col_idx, col in enumerate(single_df.columns):
                        chart_data.add_series(col,
                                              (single_df.ix[:, col_idx].values))
...
def copy_chart_properties(shp_copy_from, sld_rep):
    original_shapes_chart_type = str(shp_copy_from.chart.chart_type).split(" ")[
        0]
    ...
    add_bar_chart(
            ...
            left=shp_copy_from.left, top=shp_copy_from.top,
            width=shp_copy_from.width, height=shp_copy_from.height,
