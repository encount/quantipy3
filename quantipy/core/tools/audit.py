...
VALID_CONVERT = {'float':
                 ...
                 'string':
                     ['single', 'delimited set', 'int', 'float', 'date',
                      'string']
                 }
...
class Audit(object):
    ...
    def __getitem__(self, ds):
        ...
        if isinstance(ds[0], str):
            not_incl = [d for d in ds if
                        not d in self.ds_names + list(self.ds_alias.values())]
            if not_incl:
    ...
    def __contains__(self, ds):
        for d in ds:
            if not d in list(self.ds_alias.keys()) or d in list(
                    self.ds_alias.values()):
                ...
                if var in var_map[var.lower()].get(n, []):
                    ...
                    for v in self.all_incl_vars:
                        if v == var:
                            ...
                            break
                        elif var.lower() in v.lower() and n in var_map[
                            v.lower()]:
    ...
    def rename_by(self, name, datasets='all', ignore=[]):
        ...
        if not datasets:
            ...
            datasets = [alias for alias in list(self.ds_alias.values()) if
                        not alias == name]
        else:
            datasets = [ds for ds in self._get_alias(datasets) if
                        not ds == name]
        for ds in datasets:
    ...
    def fill_mismatches_by(self, name='all', datasets='all', ignore=[]):
        ...
        for n in self._get_alias(name):
            ...
            if not datasets:
                ...
                use_ds = [ds for ds in list(self.ds_alias.values()) if
                          not ds == n]
            else:
            ...
            for x, n1 in enumerate(list(self.ds_alias.values()), 1):
                for n2 in list(self.ds_alias.values())[x:]:
                    if all(self[n].var_exists(v) for n in [n1, n2]):
                        collection = 'masks' if self[n1].is_array(
                            v) else 'columns'
                        tobj1 = self[n1]._meta[collection][v]['text']
                        tobj2 = self[n2]._meta[collection][v]['text']
                        diff_tks = self._compare_texts(tks, tobj1, tobj2,
                                                       strict)
                        header['{},\n{}'.format(n1,
                                                n2)] = diff_tks if diff_tks else ''
                    else:
                        header['{},\n{}'.format(n1, n2)] = ''
    ...
    def show_items(self, array, text_key=None):
        ...
        for a in array:
            ...
            for name in list(self.ds_alias.values()):
                ...
                if a in ds:
                    if label:
                        val = [ds.text(s, True, text_key, etk) for s in
                               ds.sources(a)]
                        ind = ds.sources(a)
                    else:
                        val = ds.sources(a)
    ...
    def report_item_diffs(self):
        ...
        for a in list(total_ais.keys()):
            ...
            same_order = all(
                    len([val for val in order.loc[a].T[x].unique().tolist()
                ...
                for x, n1 in enumerate(list(self.ds_alias.values()), 1):
                    for n2 in list(self.ds_alias.values())[x:]:
                        if all(self[n].var_exists(s) for n in [n1, n2]):
                            ...
                            diff = self._compare_texts(tks, tobj1, tobj2,
                                                       strict) or np.NaN
                        else:
                            diff = np.NaN
