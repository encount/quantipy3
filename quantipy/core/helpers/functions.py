...
def verify_dtypes_vs_meta(data, meta):
    ...
    dtypes = data.dtypes
    dtypes.name = 'dtype'
    var_types = pd.DataFrame({k: v['type'] for k, v in meta['columns'].items()},
                             index=['meta']).T
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def as_datetime64(data, date_format='dmy', date_sep='/', time_format='hm',
                  time_sep=':'):
...
def summarize_time(data, raw_output=True):
    ...
    if not raw_output:
        df.index = pd.MultiIndex.from_product([[data.name], df.index],
...
def single_value_counts(data):
    ...
    data = data.dropna()
    df = pd.DataFrame(
            {c: data[data[data.columns[0]] == c][data.columns[1]].sum() for c in
             data[data.columns[0]].astype(int).unique()}, index=['@1'])
    if margin == 0:
...
def calc_margins(data, multi=False):
    ...
    if multi:
        ...
        # margin_values = data
        margin_values = data[data[data.columns[0]].notnull()][
            data.columns[1]].sum()
...
def delimited_set_value_counts(data, value_map=None):
    ...
    df = pd.DataFrame({k: data[
        data[data.columns[0]].str.contains("(^{k};)|(;{k};)".format(k=k),
                                           regex=True)][data.columns[1]].sum()
                       for k in value_map}, index=['@1'])
    df['All'] = margin
    return df.T
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def array_value_counts(data, array_name, array_items, weight='@1',
                       aggfunc='sum'):
    ndf = pd.concat([data.pivot_table(values=weight, index=[p], margins=True,
                                      aggfunc=aggfunc) for p in array_items],
                    axis=1, keys=array_items)
    ndf.index = pd.MultiIndex.from_product([[array_name], ndf.index],
                                           names=['Array', 'Values'])
    ndf.columns = pd.MultiIndex.from_product([[array_name], ndf.columns],
                                             names=['Array', 'Item'])
    return ndf
    ...
    """ Takes an array and creates a list of combinations from it.
        ...
        example:
            ...
            [('A', 'B'),
             ...
             ('C', 'B')]
    """
    return [(item_1, item_2) for item_1 in array for item_2 in array if
            item_1 is not item_2]


# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def array_items_from_meta(meta, variable):
    if variable in meta['masks']:
        return [var['source'].split('@')[1] for var in
                meta['masks'][variable]['items'] if 'source' in var]
    else:
        return []
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def update_view_meta(view_df, meta, method_name, name, fullname, method_type,
                     label='', source=None):
...
def describe(data, x, weights=None):
    ...
    return pd.DataFrame(desc_df[['Count', 'Eff. count', 'Min', 'Max', 'Mean',
                                 'StdDev', 'Weights squared sum',
                                 'Efficiency']])
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def create_multiindex_from_length(name, length, names=['Question', 'Values'],
                                  margins=True):
    if margins:
...
def is_nested_with_arrays(link):
    if '>' in link.y + link.x:
        ...
        variable_masks = [mask for mask in [key for key in masks if
                                            'array' in masks[key]['type']] if
                          mask in variables]
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def categorical_value_counts(df, is_single=False, x_is_multi=False,
                             y_is_multi=False):
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if len(df.dropna().index) == 0:
            ...
            if x_is_multi and y_is_multi:
                ...
                df = df.dropna()
                ct = pd.crosstab(index=df[df.columns[0]].astype(int),
                                 columns=df[df.columns[1]].astype(int),
                                 values=df[df.columns[2]], aggfunc='sum')
    ...
    ct.index = pd.MultiIndex.from_product(
            [[df.columns[0]], ct.index.astype(str)],
            names=['Question', 'Values'])
    ct.columns = pd.MultiIndex.from_product(
            [[df.columns[-2]], ct.columns.astype(str)],
            names=['Question', 'Values'])
...
def hide_codes_x(x, codes_to_hide, views, exclude_from_base=False):
    ...
    *** NOT IMPLEMENTED YET ***
    ...
    for y in x:
        for view in views:
            if view in x[y]:
                x[y][view].meta.update(
                        {'x_hidden_codes': create_range_as_list(codes_to_hide),
...
def reorder_codes_x(x, new_order, views):
    ...
    for y in x:
        for view in views:
            if view in x[y]:
                x[y][view].meta.update(
                        {'x_new_order': create_range_as_list(new_order),
...
def define_multicodes(varlist, meta):
    multicodes = {}
    for var in varlist:
        multicodes.update({var: [mrs_q for mrs_q in meta['columns'] if
                                 mrs_q.startswith(var + '_')]
                           })
...
def create_delimited_from_single(data, meta=None, mrs_spec={}):
    ...
    for name, definition in list(mrs_spec.items()):
        ...
        if not meta is None:
            ...
            meta['columns'][mrs_name] = create_column(mrs_name,
                                                      type_name='delimited set',
                                                      text=q_lab, values=values)
    ...
    if not isinstance(index, pd.MultiIndex):
        ...
        if not levels_map:
            ...
            if not len(levels_map) == len(index.levels):
                ...
                mapped_levels = [
                    [levels_map[l][i] if i in list(levels_map[l].keys()) else i
                     for i in level] for l, level in enumerate(index.levels)]

        if not names_map:
            mapped_names = index.names
        else:
            mapped_names = [names_map[n] if n in list(names_map.keys()) else n
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def df_to_value_matrix(data, x, y=None, limit_x=None, limit_y=None,
                       weights=None):
    '''
    ...
    if not y is None:
        ...
        if weights:
            value_matrix = np.concatenate((wg_vec, x_matrix * wg_vec, y_matrix),
                                          axis=1)
        else:
...
def make_default_cat_view(data, x, y=None, weights=None):
    ...
    '''
    matrix, x_ref, y_ref = df_to_value_matrix(data=data, x=x, y=y,
                                              weights=weights)
    ...
    if y is None or y == '@':
        ...
        if len(data.index) == 0:
            ...
            if data[y].dtype == 'object':
                ...
                df = pd.concat(
                        [describe(data[data[y] == y_code], x, weight) for y_code
                         in y_codes], axis=1)
                df.columns = [
...
def partition_view_df(view, values=False, data_only=False, axes_only=False):
    ...
    '''
    df = view.copy()
    index = df.index.droplevel() if isinstance(df.index,
                                               pd.MultiIndex) else df.index
    columns = df.columns.droplevel() if isinstance(df.columns,
                                                   pd.MultiIndex) else df.columns
...
def calc_net_values(link, source_view, combine_codes, force_raw_sum=False):
    ...
    if not source_view.meta['x']['is_multi'] or force_raw_sum:
        ...
        if not link.y == '@':
            ...
            xcodes = len(x_def) + 1
            net_values = np.sum(
                    matrix[matrix[:, 1:xcodes].sum(axis=1) > 0][:, 0])
        ...
        # what is this else-branch supposed to achieve?
            ...
            try:
                if item in meta['columns'] and meta['columns'][item]['parent']:
                    items.append(
                            list(meta['columns'][item]['parent'].keys())[0])
            except:
                if 'masks@{}'.format(re.sub(pattern, '', item)) in \
                        meta['sets'][based_on]['items']:
